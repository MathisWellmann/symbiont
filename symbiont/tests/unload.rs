    ...
    std::fs::read_dir(rt.crate_dir())
        ...
        .find(|path| {
            path.file_name()
                ...
                .is_some_and(|name| name.starts_with("libsymbiont_evolvable") && name.contains(&needle))
        })
        .unwrap_or_else(|| rt.crate_dir().join(format!("libsymbiont_evolvable{needle}missing")))
}
...
async fn revisions_can_be_unloaded() {
    ...
    for prompt in ["plus 5", "plus 7", "plus 9"] {
        revs.push(rt.evolve(&agent, prompt).await.expect("Can evolve").revision());
    }
    let [rev_5, rev_7, rev_9] = revs[..] else {
        unreachable!("three evolutions")
    };
    assert_eq!((rev_5, rev_7, rev_9), (Revision::new(1), Revision::new(2), Revision::new(3)));
    ...
    assert!(is_mapped(&so_5), "sanity: the dylib is mapped before unload");
    ...
    assert!(unload_step_fn(rev_5).is_none(), "no new handles into a tombstone");
    let err = rt
    ...
    assert_eq!(rt.active_revision(), rev_9, "the refusal leaves the active revision alone");
    ...
    assert!(so_5.exists(), "the file stays while a handle pins the dylib");
    ...
    assert_eq!(outcome, Unloaded::Now);
    assert!(!so_7.exists(), "the file is deleted before the call returns");
    if cfg!(target_os = "linux") {
    ...
    rt.activate_revision(Revision::INITIAL).expect("Can activate");
    counter = 0;
    unload_step(&mut counter);
    assert_eq!(counter, 1);
    ...
    assert_eq!(
        counter_value(&snapshot, observability::REVISION_UNLOADS, ("outcome", "now")),
        2,
        "revisions 2 and 3 were unmapped immediately"
    );
    assert_eq!(
        counter_value(&snapshot, observability::REVISION_UNLOADS, ("outcome", "pinned")),
        1,
